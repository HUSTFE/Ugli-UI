...
function Swiper(container, options = {}) {
  ...
  let currentTranslate = circle(this.index) * slideWidth // cache the wrapper translate for smooth swipe.
  const delta = { x: 0, y: 0 }
    ...
    if (e.touches.length === 1) {
      ...
      if (!isPastBounds) {
        wrapper.style.transform =
          `translate3d(${delta.x - currentTranslate}px, 0px, 0px)`
      } else {
        const sign = Math.abs(delta.x) / delta.x
        wrapper.style.transform =
          `translate3d(${(sign * (Math.abs(delta.x) ** (1 - (this.options.resistance / 2)))) - currentTranslate}px, 0px, 0px)`
      }
    }
  }
  ...
  const onTouchEnd = (e) => {
    if (e.changedTouches.length === 1) {
      ...
      currentTranslate = circle(this.index) * slideWidth
    }
  }
